  ...
  var adapter = {
        ...
        client.query(query, function(err, result) {
          ...
          client.query(autoIncrementQuery, function(err, aResult) {
            ...
            client.query(indiciesQuery, function(err, iResult) {
              ...
              // XXX Hack: remove deletedAt from schema returned to keep
              // Waterline from thinking that deletedAt was inadvertantly
              // added and thusly removing the deletedAt column.
              if(dbs[table].config.softDelete) {
                var resultSchema = extend(true, {}, normalizedSchema);
                if (_.has(resultSchema, 'deletedAt')) {
                  delete resultSchema['deletedAt'];
                }
    ...
    define: function(table, definition, cb) {
      ...
      spawnConnection(function __DEFINE__(client, cb) {
        ...
        // Add delete column if using soft deletes
        if(dbs[table].config.softDelete && !definition.deletedAt) definition.deletedAt = { type: 'datetime' };
    ...
    count: function(table, options, cb) {
      spawnConnection(function __COUNT__(client, cb) {

        // Check if this is an aggregate query and that there is something to return
        if(options.groupBy || options.sum || options.average || options.min || options.max) {
          return cb(new Error('Cannot count aggregate functions'));
        }
        ...
        var query = new Query(_schema).count(table, options);

        // Run Query
        client.query(query.query, query.values, function __COUNT__(err, result) {
          if(err) return cb(err);
    ...
    find: function(table, options, cb) {
      spawnConnection(function __FIND__(client, cb) {
        ...
        // Adjust query options to support soft delete by filtering deletedAt column
        if (dbs[table].config.softDelete === true) {
          if (!options) options = {};
          if (_.has(options, 'where') && (options.where !== null)) {
    ...
    destroy: function(table, options, cb) {
      spawnConnection(function __DELETE__(client, cb) {
        ...
        var softDelete = dbs[table].config.softDelete;
        ...
        // Build Query
        if (softDelete === true) {
          var query = new Query(dbs[table.replace(/["']/g, "")].schema).softDestroy(table, options);          
        } else {
